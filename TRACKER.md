...
- [ ] **W4-8b Windows capture backend — scoped, deliberately not rushed**: ~500 lines of native Rust (`windows` crate: Graphics.Capture + D3D11 + DXGI interop) — `appcast_list_windows` via EnumWindows is easy, but `appcast_start/set_bounds` needs a GraphicsCaptureItem-from-HWND interop, a Direct3D11CaptureFramePool, and a child-HWND swapchain composited + bounds-synced over the slot div (the Windows twin of the mac CALayer float), with real DPI/resize/occlusion edge cases. `cargo check` can't verify any of the runtime behavior — this arc needs live on-machine iteration, so it ships as its own session(s) rather than a blind landing. Phase A = mirror-only; input forwarding later. Unlocks Mirror Wall / cast-to-chat / PiP on Windows.
